...
export async function fetchMergedAgenda(calendars) {
  const now = new Date();
...
export async function getCachedAgenda(supabase, userId, settings, force, maxAgeMin) {
  ...
  const events = await fetchMergedAgenda(settings.calendars || []);
  settings.agendaCache = { ts: Date.now(), events };
  await saveSettings(supabase, userId, settings);
  return events;
}

export function todayEventsCompact(events) {
  const now = new Date();
  const today = events.filter((e) => new Date(e.start).toDateString() === now.toDateString()).slice(0, 15);
